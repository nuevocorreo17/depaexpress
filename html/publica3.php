<?php
	include('header.php');
?>
	<section class="seccion-sinbanner pasos">
		<div class="container">
			<div class="row">
				<div class="col-12 col-xl-8 offset-xl-2 columna-pasos">
					<div class="circulo-paso ">
						<span class="circulo" >1</span>
						<h4 class="titulo-letra-azul">Tipo de aviso</h4>
					</div>
					<div class="circulo-paso">
						<span class="circulo" >2</span>
						<h4 class="titulo-letra-azul">Datos del aviso</h4>				
					</div>
					<div class="circulo-paso activo">
						<span class="circulo" >3</span>
						<h4 class="titulo-letra-azul">tu aviso</h4>
					</div>
				</div>
			</div>
		</div>
	</section>
	<section class="paso modelo">
		<div class="container">
			<div class="row">
				<div class="col-12 col-xl-10 offset-xl-1">
					<div class="row">
						<div class="col-12 col-xl-8 offset-xl-2  precio-principal">
							<h2 class="titulo-letra-azul">ANUNCIO EXPRESS</h2>
							<h2 class="titulo-letra-azul">s/ 000</h2>
							<p>Duración 30 días de publicación, Posición Inferior en resultados, 20 Fotos</p>
						</div>
					</div>
					<div class="row fila-previo">
						<div class="col-12 col-xl-4 columna-imagen">
							<img src="assets/img/foto-inmueble-previo.jpg" class="imagen-previo">
							<span class="label">COMPRA</span>
						</div>
						<div class="col-12 col-xl-8 columna-detalle">
							<div class="row">
								<div class="col-12">
									<div class="row">
										<div class="col-12 col-xl-8">
											<h2 class="nombre-proyecto">Proyecto Edificio Foresta</h2>
											<p class="direccion"><i class="fas fa-map-marker-alt"></i> Calle Calderón de la Barca 177 - ATE</p>
											<ul class="caracteristica">
												<li><i class="fas fa-bed"></i><span class="dato">0 a 0 Dorm.</span></li>
												<li><i class="fas fa-shower"></i><span class="dato">0 a 0</span></li>
												<li>50.48 - 78.21 m2</li>
											</ul>
										</div>
										<div class="col-12 col-xl-4 bloque-azul">
											<p>(US$ 204,671)</p>
											<p>Desde</p>
											<h2 class="precio-inmueble">S/ 689,742</h2>
											
										</div>
									</div>
									<div class="row">
										<div class="col-12 col-xl-6">
											<p class="descripcion">Arquitectura de vanguardia a una cuadra del parque Pershing y el parque Junin, a 4 cuadras de Plaza San Miguel, una zona tranquila y céntrica ideal para empezar una nueva vida en armonía.</p>
										</div>
										<div class="col-12 col-xl-6 columna-lista-logos">
											<ul class="lista-logos">
												<li><img src="assets/img/inmobiliaria1.png" class="log"></li>
												<li><img src="assets/img/inmobiliaria1.png" class="log"></li>
											</ul>
										</div>
									</div>
									<div class="row">
										<div class="col-12 col-xl-4">
											<a href="#" class="link">ver planos</a>
										</div>
										<div class="col-12 col-xl-4 offset-xl-4">
											<a href="#" class="boton">contactar</a>
										</div>
									</div>	
								</div>
							</div>
						</div>
					</div>
					<div class="row centrado">
						<div class="col-12  text-center">
							<a href="#" class="boton">PUBLICAR</a>
						</div>
					</div>
				</div>
			</div>
		</div>
	</section>
<?php
	include('footer.php');
?>